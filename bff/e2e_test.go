...
func TestEndToEndBrowserFlow(t *testing.T) {
	...
	resp, err := client.Get(bffSrv.URL + "/bff/login?return_to=/apps")
	if err != nil {
	...
	resp, err = client.Get(bffSrv.URL + "/bff/session")
	if err != nil {
	...
	resp, err = client.Get(bffSrv.URL + "/api/admin/users")
	if err != nil {
	...
	resp, err = client.Post(bffSrv.URL+"/api/admin/apps", "application/json", strings.NewReader("{}"))
	if err != nil {
	...
	req, _ := http.NewRequest(http.MethodPost, bffSrv.URL+"/api/admin/apps", strings.NewReader("{}"))
	...
	req, _ = http.NewRequest(http.MethodPost, bffSrv.URL+"/bff/logout", nil)
	req.Header.Set("X-CSRF-Token", sess.CSRF)
	resp, err = client.Do(req)
	if err != nil {
